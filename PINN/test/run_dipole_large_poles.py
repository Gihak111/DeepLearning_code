...
# Two large circular poles
# Positive pole: center (0, 0.4), radius 0.2, V = 1.0
# Negative pole: center (0, -0.4), radius 0.2, V = -1.0

# 1. Positive pole boundary & interior points
...
# 2. Negative pole boundary & interior points
...
V_u = torch.FloatTensor(np.vstack((V_g, V_p, V_far) if False else np.vstack((V_pos, V_neg, V_outer))))
...
for epoch in range(2000):
    ...
    optimizer.step()
    if (epoch + 1) % 400 == 0:
        print(f"Epoch {epoch+1}, Loss: {loss.item():.4f} (BC: {Loss_BC.item():.6f}, PDE: {Loss_PDE.item():.4f})")

# Verify predictions
print("\n--- Dipole predictions at key points ---")
with torch.no_grad():
    print(f"V(0.0, 0.4) [Positive Center, Target=1.0]: {model(torch.FloatTensor([[0.0, 0.4]]))[0].item():.4f}")
    print(f"V(0.0, -0.4) [Negative Center, Target=-1.0]: {model(torch.FloatTensor([[0.0, -0.4]]))[0].item():.4f}")
    print(f"V(0.0, 0.0) [Symmetry Line, Target=0.0]: {model(torch.FloatTensor([[0.0, 0.0]]))[0].item():.4f}")
    print(f"V(0.0, 0.7) [Above Positive]: {model(torch.FloatTensor([[0.0, 0.7]]))[0].item():.4f}")
