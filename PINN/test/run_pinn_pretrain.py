...
# Stage 1: Supervised Pre-training (BC only)
print("=== Stage 1: Pre-training on BC loss (1000 epochs) ===")
...
for epoch in range(1000):
    ...
    optimizer_pre.step()
    if (epoch + 1) % 200 == 0:
        print(f"Epoch {epoch+1}, BC Loss: {loss_bc.item():.6f}")

with torch.no_grad():
    print(f"After pre-training V(0.0, 0.0) [Ground]: {model(torch.FloatTensor([[0.0, 0.0]]))[0].item():.4f}")
    print(f"After pre-training V(0.0, 0.2) [Patch]: {model(torch.FloatTensor([[0.0, 0.2]]))[0].item():.4f}")

# Stage 2: PINN Training (BC + PDE)
print("\n=== Stage 2: Full PINN Training (2000 epochs) ===")
optimizer_full = optim.Adam(model.parameters(), lr=5e-4) # slightly lower learning rate
