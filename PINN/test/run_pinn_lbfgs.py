...
# Option 1: Adam with larger BC weight (e.g. 100.0)
print("=== Option 1: Adam with BC Weight = 100.0 ===")
hidden_layers = [64, 64, 64, 64, 64]
model_adam = DynamicTanhPINN(input_size=2, hidden_layers=hidden_layers, output_size=1)
optimizer_adam = optim.Adam(model_adam.parameters(), lr=1e-3)

for epoch in range(2000):
    optimizer_adam.zero_grad()
    u_pred_bc, _ = model_adam(X_u)
    Loss_BC = nn.MSELoss()(u_pred_bc, V_u)
    Loss_PDE = calc_pde_loss(model_adam, X_f)
    loss = 100.0 * Loss_BC + Loss_PDE
    loss.backward()
    optimizer_adam.step()

with torch.no_grad():
    print(f"V(0.0, 0.0) [Ground, Target=0.0]: {model_adam(torch.FloatTensor([[0.0, 0.0]]))[0].item():.4f}")
    print(f"V(0.0, 0.2) [Patch, Target=1.0]: {model_adam(torch.FloatTensor([[0.0, 0.2]]))[0].item():.4f}")
    print(f"V(0.0, 0.5) [Above patch]: {model_adam(torch.FloatTensor([[0.0, 0.5]]))[0].item():.4f}")

# Option 2: L-BFGS optimizer
print("\n=== Option 2: L-BFGS Optimizer (BC Weight = 20.0) ===")
model_lbfgs = DynamicTanhPINN(input_size=2, hidden_layers=hidden_layers, output_size=1)
optimizer_lbfgs = optim.LBFGS(model_lbfgs.parameters(), lr=0.1, max_iter=20, history_size=100)

for epoch in range(100):
    def closure():
        optimizer_lbfgs.zero_grad()
        u_pred_bc, _ = model_lbfgs(X_u)
        Loss_BC = nn.MSELoss()(u_pred_bc, V_u)
        Loss_PDE = calc_pde_loss(model_lbfgs, X_f)
        loss = 20.0 * Loss_BC + Loss_PDE
        loss.backward()
        return loss
    optimizer_lbfgs.step(closure)

with torch.no_grad():
    print(f"V(0.0, 0.0) [Ground, Target=0.0]: {model_lbfgs(torch.FloatTensor([[0.0, 0.0]]))[0].item():.4f}")
    print(f"V(0.0, 0.2) [Patch, Target=1.0]: {model_lbfgs(torch.FloatTensor([[0.0, 0.2]]))[0].item():.4f}")
    print(f"V(0.0, 0.5) [Above patch]: {model_lbfgs(torch.FloatTensor([[0.0, 0.5]]))[0].item():.4f}")
